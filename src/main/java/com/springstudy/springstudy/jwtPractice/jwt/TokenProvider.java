...
public class TokenProvider implements InitializingBean {
    ...
    public void afterPropertiesSet() {
        // 2. 난 이후에 secret값을 Base64 Decode해서 key변수에 할당
    ...
    public Authentication getAuthentication(String token) {
        // token을 param 으로 받아서 token으로 클레임을 만들고
        Claims claims = Jwts
            .parserBuilder()
            .setSigningKey(key)
            .build()
    ...
    public boolean validateToken(String token) {
        try {
            // Token을 파싱해보고, 발생하는 익셉션들을 캐치, 문제가 있으면 false, 정상이면 true를 반환
